...
> **Shell / chrome revamp (P14, frozen at 14.8):** [`SHELL_PLAN.md`](SHELL_PLAN.md),
> [`SHELL_PROGRESS.md`](SHELL_PROGRESS.md), visual
> [`shell-mockup.html`](shell-mockup.html).
> **Post-shell polish (P15):** [`POLISH_PLAN.md`](POLISH_PLAN.md),
> [`POLISH_PROGRESS.md`](POLISH_PROGRESS.md). Do not add P14/P15 rows here.
...
- `Composer` (**DS-13**): three-element row — mic, auto-growing textarea capped at 40vh, send. Send long-press/right-click menu: attach, schedule, AI rewrite, silent send. Mic replaces the entire row with a voice recorder (pause, resume, preview, cancel, send at any point). Reply/edit strips, a schedule indicator, and removable attachment chips share the composer surface above the row. **No mic↔send morph**
...
- Pinned-conversation and mark-as-unread affordances in `ChatListItem` (NR-21, NR-22); silent send (**NR-23**) is one item in the composer's send long-press menu (**DS-13**), alongside attach, schedule, and rewrite
...
| Check                               | How it is proven                                                                                                                                                                                               |
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Components behave                   | Vitest + Testing Library for grouping logic, tick state rendering, composer send menu and voice takeover (**DS-13**), long-press threshold                                                                     |
| Formatting is symmetric and bounded | A spec asserting the **same input renders identically** whether sent or received, and whether authored by a human or a bot; that `# heading`, a table and raw HTML each render literally; and that `           |
...
| 1.5     | **The fourteen new-feature components** and the personalisation surface: `PollCard`, `ReactionDetailsSheet`, `SelectionToolbar`, the unified picker sheet (emoji/sticker/GIF/saved-reply tabs), `LocationCard`, `ContactCard`, `TranscriptBlock`, `SlashCommandMenu`, `WallpaperPicker`, `QrSheet`, `ReportSheet`, `SessionListItem` | DESIGN_SYSTEM §4 (additions), §5.2, §8.1, §8.2, DS-8/DS-9/DS-10/DS-12/DS-13; SCHEMA §12 for the shapes they render | `botverse/src/components/chat/EmojiPickerSheet.tsx`, `MessageInfoBottomSheet.tsx`, `ForwardModal.tsx` — the sheet patterns the new components must reuse rather than reinvent; emoji stays in the picker, not as composer chrome                  |
