...
const expectedPaths = [
  ...
  "/api/v1/admin/reports/{id}/warn",
  "/api/v1/admin/settings",
  "/api/v1/admin/sticker_packs",
  "/api/v1/admin/sticker_packs/reorder",
...
describe("MSW handlers", () => {
  ...
  it("serves every generated path with typed bodies", { timeout: 15_000 }, async () => {
  ...
  it("serves bot directory, rewrite, helpers, and style-profile consent", async () => {
    ...
    const reports = await client.GET("/api/v1/admin/reports");
    expect(reports.data?.reports[0]?.reason).toBe("spam");
    const filteredReports = await client.GET("/api/v1/admin/reports", {
      params: { query: { status: "reviewing", subject_type: "bot", max_age_hours: 24 } },
    });
    expect(filteredReports.data?.reports).toHaveLength(0);
    const pendingReports = await client.GET("/api/v1/admin/reports", {
      params: { query: { status: "pending", subject_type: "message" } },
    });
    expect(pendingReports.data?.reports).toHaveLength(1);
    const report = await client.GET("/api/v1/admin/reports/{id}", { params: { path: { id: 1 } } });
    expect(report.data?.status).toBe("pending");
    const missingReport = await client.GET("/api/v1/admin/reports/{id}", {
      params: { path: { id: 9 } },
    });
    expect(missingReport.response.status).toBe(404);
    const dismissed = await client.POST("/api/v1/admin/reports/{id}/dismiss", {
      params: { path: { id: 1 } },
      body: { note: "ok" },
    });
    expect(dismissed.data?.status).toBe("dismissed");
    const warned = await client.POST("/api/v1/admin/reports/{id}/warn", {
      params: { path: { id: 1 } },
      body: {},
    });
    expect(warned.data?.status).toBe("actioned");
    const removed = await client.POST("/api/v1/admin/reports/{id}/remove_content", {
      params: { path: { id: 1 } },
    });
    expect(removed.data?.status).toBe("actioned");
    const deactivated = await client.POST("/api/v1/admin/reports/{id}/deactivate_account", {
      params: { path: { id: 1 } },
    });
    expect(deactivated.data?.status).toBe("actioned");
    const packs = await client.GET("/api/v1/admin/sticker_packs");
    expect(packs.data?.sticker_packs).toHaveLength(2);
    const createdPack = await client.POST("/api/v1/admin/sticker_packs", {
      body: { name: "Dogs", kind: "emoji" },
    });
    expect(createdPack.data?.name).toBe("Dogs");
    const createdDefault = await client.POST("/api/v1/admin/sticker_packs", { body: {} });
    expect(createdDefault.data?.name).toBe("Cats");
    const updatedPack = await client.PATCH("/api/v1/admin/sticker_packs/{id}", {
      params: { path: { id: 1 } },
      body: { published: false },
    });
    expect(updatedPack.data?.published_at).toBeNull();
    const publishedPack = await client.PATCH("/api/v1/admin/sticker_packs/{id}", {
      params: { path: { id: 1 } },
      body: { published: true },
    });
    expect(publishedPack.data?.published_at).toBe(MESSAGE_STAMP);
    const reordered = await client.PATCH("/api/v1/admin/sticker_packs/reorder", {
      body: { ids: [2, 1] },
    });
    expect(reordered.data?.sticker_packs[0]?.id).toBe(2);
    const addedSticker = await client.POST(
      "/api/v1/admin/sticker_packs/{sticker_pack_id}/stickers",
      {
        params: { path: { sticker_pack_id: 1 } },
        body: { signed_id: "signed", shortcode: "wave2" },
      },
    );
    expect(addedSticker.data?.shortcode).toBe("wave");
    const removedSticker = await client.DELETE(
      "/api/v1/admin/sticker_packs/{sticker_pack_id}/stickers/{id}",
      { params: { path: { sticker_pack_id: 1, id: 1 } } },
    );
    expect(removedSticker.data?.ok).toBe(true);
    const destroyedPack = await client.DELETE("/api/v1/admin/sticker_packs/{id}", {
      params: { path: { id: 1 } },
    });
    expect(destroyedPack.data?.ok).toBe(true);
  ...
  it(
    "serves conversation and message routes from the in-memory store",
    { timeout: 15_000 },
    async () => {
      const client = createApiClient("http://rajya.test");
      ...
      const cancelledGeneration = await client.POST(
        "/api/v1/conversations/{id}/generations/cancel",
        {
          params: { path: { id: 1 } },
      ...
      const missingUnreact = await client.DELETE(
        "/api/v1/messages/{message_id}/reactions/{emoji}",
        {
