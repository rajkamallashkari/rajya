...
RSpec.describe Auth::Otp::Request do
  ...
  it "emails an OTP when the account exists" do
    user = create(:user)

    expect { described_class.call(email: user.email) }.to change { ActionMailer::Base.deliveries.size }.by(1)
    expect(ActionMailer::Base.deliveries.last.subject).to include("your Rajya code")
  end
