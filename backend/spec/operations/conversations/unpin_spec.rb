...
RSpec.describe Conversations::Unpin do
  it "clears pinned_at for the actor only" do
    user = create(:user)
    conversation = create_direct_between(user.account, create(:account))
    Conversations::Pin.call(account: user.account, conversation: conversation)
    described_class.call(account: user.account, conversation: conversation)

    expect(Conversations::View.for(conversation.reload, user.account).membership.pinned_at).to be_nil
  end
